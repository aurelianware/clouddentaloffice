...
Configure `CloudHealthOffice:BaseUrl`, `CloudHealthOffice:EstimatePath`, and `CloudHealthOffice:Enabled` through environment configuration. Map each CloudDentalOffice payer ID to the CloudHealthOffice benefit-plan GUID under `CloudHealthOffice:BenefitPlanMappings`; for example `CloudHealthOffice__BenefitPlanMappings__PAYER1=<guid>`. If an API key is required, provide `CloudHealthOffice__ApiKey` through the deployment secret store; never commit it.
